...
func TestTickerLogger(t *testing.T) {
	writer := &bytes.Buffer{}
	timer := NewTickerLogger(time.Hour, LevelDebug, writer)
	timer.Debug("debug")
	timer.Info("info")
	timer.Warn("warn")
	timer.Error("error")
	...
	writer = &bytes.Buffer{}
	timer = NewTickerLogger(time.Microsecond, LevelDebug, writer)
...
func TestTickerLoggerInterval(t *testing.T) {
	writer := &bytes.Buffer{}
	timer := NewTickerLogger(time.Second, LevelDebug, writer)
